...
class ProductController extends Controller
{
     ...
     * @return Response
     */
    public function index(Cart $cart, $activeProductId = null, $activeSize = null)
    {
        $data = Product::with('sizes')->get()->map(function (Product $product) use ($cart, $activeProductId, $activeSize) {
            $product->thumb = null;
            ...
            if ($activeSize && $activeProductId) {
                $activeCartItem = $cart->content()->firstWhere('id', $activeProductId.'-'.$activeSize);
            } else {
                $activeCartItem = null;
            }
        ...
        return view('public.product.index', compact('data', 'activeSize', 'activeProductId'));
    }
