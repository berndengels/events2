                        ...
                        <div class="row">
                            ...
                            <div class="col-12 col-lg-8 mh-100">
                                <div id="data" class="ml-1 sidenav">
                                    <h5 class="info mt-2 mt-sm-0 ml-sm-2">{{ $title }}</h5>
                                    <x-form id="frmMenu"
                                            method="post"
                                            :action="route('admin.menuStore')">
                                        <x-form-input name="id" id="id" type="hidden" />
                                        <x-form-checkbox name="is_published" id="is_published" label="Veröffentlixhen" />
                                        <x-form-checkbox name="api_enabled" id="api_enabled" label="API erlaubt" />
                                        <x-form-input name="name" id="name" label="Name" />
                                        <x-form-input name="css_class" id="css_class" label="CSS-Klassen" />
                                        <x-form-input name="icon" id="icon" label="Icon" />
                                        <x-form-input name="fa_icon" id="fa_icon" label="Font-Awesome-Icon" />
                                        <x-form-select name="menuItemType" id="menuItemType" label="Type wählen" :options="$menuItemTypeOptions" />
                                        <x-form-select name="route" id="route" label="Routen" :options="$publicRoutes" class="d-none" />
                                        <x-form-input name="url" id="url" label="URL" class="d-none" />
                                        <x-form-input name="submit" type="button" id="btnSubmit" value="save"
                                                      class="btn btn-primary col-12 col-sm-auto">
                                            Speichern</x-form-input>
                                        <x-form-input name="remove" type="button" id="btnRemove" value="remove"
                                                      class="btn btn-danger col-12 col-sm-auto mt-xs-2 float-right d-none">
                                            Löschen</x-form-input>
                                    </x-form>
                                </div>
                            </div>
                        </div>
...
//                                            $('#btnRemove','#frmMenu').removeClass('d-none');
    ...
    function handleMenuItemType(type) {
        ...
        switch(parseInt(type, 10)) {
            case 1:
                // only label
                $route.prop('disabled','disabled').removeClass('d-none').addClass('d-none');
                $url.prop('disabled','disabled').removeClass('d-none').addClass('d-none');
                break;

            case 2:
                // external link
                $route.prop('disabled','disabled').removeClass('d-none').addClass('d-none');
                $url.removeAttr('disabled').removeClass('d-none');
                break;

            case 3:
                // internal route
                $route.val($url.val()).removeAttr('disabled').removeClass('d-none');

                $url.val($route.val()).removeClass('d-none');
